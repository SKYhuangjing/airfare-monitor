...
桌面 R3 实现使用独立 `desktop_mail` 非敏感配置和 Windows 凭据库，保留现有 CLI 环境变量邮件入口。桌面测试邮件仅由用户点击后在后台线程发送；桌面发送失败时价格、数据库和 Excel 已先保存，并产生脱敏应用事件。人工确认页也只在用户点击后打开同一隔离 Profile 的临时可见浏览器，不修改原有 `browser.headless`。
